...
#forward function
#argument:W:TT decomposition elements f:feature map
def inner(weights_TT, images):
    ...
    for rank in range(0, R-1):
        ...
        for t in range(0, T-1):
            temp = temp * torch.matmul(weights_TT[t, :, rank], f[:, :, t])
        y_pred[:, rank] = temp
...
def g_inner(weights_TT,images):
    ...
    for rank in range(0, R-1):
        ...
        for t in range(1, T-1):
            temp = torch.max(temp, torch, matmul(weights_TT[t, :, r], f[:, :, t]))
        y_pred[:, rank] = temp
...
#randomly generate the component of TT decomposition
weights_TT = torch.randn(T, M, R)
for t in range(0, T-1):
    weights_TT[t, :, :] = Variable(torch.randn(R, M, device=device, dtype=dtype), requires_grad=True)
    ...
    if sss == "1":
        for epoch in range(epoch_num):
            for i, data in enumerate(trainloader, 0):
                ...
                y_predict = inner(weights_TT, inputs)
                ...
                with torch.no_grad():
                    for i in range(0, T-1):
                        weights_TT[i, :, :] -= learning_rate * weights_TT[i, :, :].no_grad
                    for i in range(0, T-1):
                        weights_TT[i, :, :].grad.zero_()

        print('Finished Training')

    else:
        param_list=[]
        param_list.append(weights_TT)
...
#Test part
    ...
    with torch.no_grad():
        for data in testloader:
            images, labels = data
            outputs = inner(weights_TT, images)
...
if str == '2':
    ...
    if sss == "1":
        for epoch in range(epoch_num):
            ...
            for i, data in enumerate(trainloader, 0):
                ...
                y_predict = g_inner(weights_TT, inputs)
                loss = (y_predict - y).pow(2).sum()
                print(epoch, i, "loss:", loss.item())
                ...
                with torch.no_grad():
                    for p in range(0, T-1):
                        weights_TT[i, :, :] -= learning_rate * weights_TT[p, :, :].no_grad
                    for q in range(0, T-1):
                        weights_TT[q, :, :].grad.zero_()
 ...
 #Test part
    ...
    with torch.no_grad():
        for data in testloader:
            images, labels = data
            outputs = g_inner(weights_TT, images)
            _, predicted = torch.max(outputs.data, 1)
            total += labels.size(0)
            correct += (predicted == labels).sum().item()
